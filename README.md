...
By default, Skatepark immediately transitions Structure Runs from the `QUEUED` state to the `RUNNING` state.
If you need artificially create a delay in order to test the behavior of your Structure when it is in the `QUEUED` state, you can do so by setting the `GT_CLOUD_RUN_DELAY` environment variable.
For example, a value of `GT_CLOUD_RUN_DELAY=5` will cause Skatepark to wait 5 seconds before transitioning the Structure Run from `QUEUED` to `RUNNING`.
